...
* **Git**: Use `git-for-bots` instead of `git` for all interactions (commits, pushing, etc.) so that commits are properly attributed to the bot account and use the correct credentials. If `git-for-bots` is not available, ask the user what to do.
  * **Sign Commits**: Always cryptographically sign commits with `git-for-bots commit -S` (unless GPG signing fails due to missing bot keys, in which case drop the `-S` flag). A successful commit is sufficient; do not run `git verify-commit`. Prompt the user if signing fails for reasons other than missing keys.
  * **Pushing**: Prompt user before pushing, unless explicit session permission is granted.
  * **Commit Message Construction**: Pass each commit-message paragraph as a separate argument. Use `git-for-bots commit -S -m "<subject>" [-m "<body>"] --trailer "Assisted-by: <agent>:<model> [tools]"`; never place `\n` escape sequences inside a `-m` argument because Git records them literally. After committing, inspect the exact message with `git-for-bots log -1 --format=%B`. See the [`git commit` documentation](https://git-scm.com/docs/git-commit).
...
* **GitHub**: Use `gh-for-bots` instead of `gh` for all interactions (PRs, issues, repos, releases, gists). It supports all the same arguments as the `gh` cli tool but runs within the bot's permissions. If `gh-for-bots` is not available, ask the user what to do.
