...
# Your Task
Iteratively improve the `Merge` logic in `merge.py`. 
...
- Use the pre-implemented helper functions `parse_svg_path` and `parse_transform` in `merge.py` to extract coordinates and parse SVG transforms.
